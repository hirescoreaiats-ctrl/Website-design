...
function TechHiringHomePage() {
  return (
    <div className="commandHome techHome">
      ...
      <AboutCommandSection />
      <HomepageFaqSection />
      <section className="commandCta techFinalCta"><div className="commandContainer commandCtaBox"><span className="sectionKicker"><Sparkles size={13} /> One connected hiring workflow</span><h2>Your Next Tech Hire <span>Starts Here</span></h2><p>Bring sourcing, candidate intelligence, hiring actions and pipeline visibility into one connected workflow.</p><div className="commandActions"><a className="commandButton commandButtonPrimary" href={PILOT_MAILTO}>Start Free Pilot <ArrowRight size={17} /></a><Link className="commandButton commandButtonGhost" href="/contact">Book a Demo</Link></div></div></section>
    </div>
  )
}
    ...
    <section className="techHero">
      <div className="commandContainer techHeroGrid">
        <div className="techHeroCopy">
          ...
          <h1>{HOME_H1}</h1>
          <p>HireScoreAI connects candidate sourcing, AI-powered evaluation, Action Agents, hiring workflows and analytics to move technical candidates from requirement to joining.</p>
...
function HiringJourney() {
  const journey = [
    ['Requirement', 'Senior Software Engineer', 'Bengaluru · 5–8 years', BriefcaseBusiness], ['Sourcing', 'Candidate pipeline', '128 candidates', Network],
    ['Screening', 'Role-fit evaluation', '84 candidates evaluated', SearchCheck], ['Shortlisting', 'Strong profiles', '16 candidates prioritized', BadgeCheck],
    ['Interviews', 'Interview workspace', '6 active interviews', CalendarCheck], ['Offer', 'Offer tracking', '2 offers in progress', MailCheck],
    ['Joining', 'Hiring closure', '1 candidate confirmed', CheckCircle2],
  ]
  const [active, setActive] = useState(0)
  const ActiveIcon = journey[active][3]
  return <section className="techStorySection hiringJourney" id="hiring-journey"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker">Complete hiring journey</span><h2>From Requirement to Joining</h2><p>One connected path from requirement to hire.</p></div><div className="journeyShell"><div className="journeyStages" role="tablist" aria-label="Hiring journey stages">{journey.map(([label], index) => <button type="button" role="tab" aria-selected={active === index} className={active === index ? 'isActive' : ''} onClick={() => setActive(index)} key={label}><span>{String(index + 1).padStart(2, '0')}</span><strong>{label}</strong></button>)}</div><div className="journeyContext" role="tabpanel"><div className="journeyContextIcon"><ActiveIcon size={20}/></div><div><span>{journey[active][0]}</span><h3>{journey[active][1]}</h3><p>{journey[active][2]}</p></div><div className="journeyContextStatus"><small>Workflow progress</small><div className="journeyProgress" aria-hidden="true"><i style={{ width: `${((active + 1) / journey.length) * 100}%` }} /></div><b>Stage {active + 1} of {journey.length}</b></div></div></div></div></section>
}
...
function HiringCommandCenter() {
  const columns = [
    ['New Candidates', [['Ishaan Shah','78'],['Kavya Nair','76']]], ['Screened', [['Neha Kapoor','84'],['Arjun Bose','82']]],
    ['Shortlisted', [['Aditi Rao','91'],['Rahul Mehta','87']]], ['Interview', [['Aditi Rao','91'],['Rahul Mehta','87']]],
    ['Offer', [['Aditi Rao','91']]], ['Joining', [['Aditi Rao','91']]],
  ]
...
function HiringAnalytics() {
  return <section className="techStorySection hiringAnalyticsSection" id="hiring-analytics"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker"><BarChart3 size={13}/> Hiring analytics</span><h2>Understand Your Hiring Pipeline</h2><p>See how candidates move through hiring, where progress slows down and what needs attention.</p></div><div className="analyticsWorkspace"><header><div><small>Hiring analytics</small><strong>Senior Software Engineer</strong></div><nav><button className="isActive" type="button">Overview</button><button type="button">Pipeline</button><button type="button">Sources</button></nav><span>Illustrative role workspace</span></header><div className="analyticsWorkspaceGrid"><section className="analyticsFunnel"><small>Hiring funnel</small><div>{[['Candidates','128'],['Screened','84'],['Shortlisted','16'],['Interviewed','6'],['Offered','2'],['Joined','1']].map(([label,value],index)=><span key={label} style={{'--funnel-size':`${100-index*10}%`}}><b>{value}</b><em>{label}</em></span>)}</div></section><section className="analyticsRoles"><small>Active roles</small>{[['Senior Software Engineer','128'],['Data Engineer','64'],['Product Engineer','41']].map(([role,count])=><div key={role}><span><strong>{role}</strong><small>{count} candidates</small></span><i><b style={{width:`${Math.min(Number(count)/1.28,100)}%`}}/></i></div>)}</section><section className="analyticsHealth"><small>Pipeline health</small><ul><li><span>Waiting for review</span><b>8</b></li><li><span>Interviews pending</span><b>3</b></li><li><span>Offers pending</span><b>1</b></li><li><span>Inactive too long</span><b>4</b></li></ul></section><section className="analyticsSources"><small>Candidate intake sources</small><div>{[['Career Page','46'],['Job Portals','38'],['Referral','24'],['Recruiter Upload','20']].map(([source,count])=><span key={source}><strong>{source}</strong><i><b style={{width:`${Number(count)*2}%`}}/></i><em>{count}</em></span>)}</div></section></div></div></div></section>
}

function JourneyCapabilities() {
  const items = [[BriefcaseBusiness, 'Job Creation', 'Create and manage hiring requirements.', '/product/create-job'], [Globe2, 'Public Apply Pages', 'Launch role-specific application pages.', '/product/public-apply-page'], [Files, 'Resume Intake', 'Collect and organize candidate resumes.', '/product/resume-upload'], [SearchCheck, 'AI Resume Screening', 'Evaluate candidates against role requirements.', '/product/ai-resume-parsing'], [Trophy, 'Candidate Ranking', 'Prioritize stronger profiles.', '/product/ai-candidate-ranking'], [MessageSquareText, 'Candidate Communication', 'Keep shortlisted candidates moving.', '/product/candidate-communication'], [CalendarCheck, 'Interview Scheduling', 'Coordinate interview steps.', '/product/interview-scheduling'], [ClipboardCheck, 'AI Screening Tests', 'Add candidate assessments where needed.', '/product/ai-screening-test']]
  return <section className="techStorySection techCapabilities"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker">Supporting product capabilities</span><h2>Built for the work between every hiring stage.</h2><p>Practical AI recruitment software for candidate intake, evaluation, progression and hiring coordination.</p></div><div className="journeyCapabilityGrid">{items.map(([Icon,title,text,href]) => <Link href={href} key={title}><Icon size={21} /><span>{title}</span><p>{text}</p><em>Explore <ArrowRight size={13}/></em></Link>)}</div></div></section>
}
    ...
    <section className="commandSection aboutCommandSection" id="about">
      <div className="commandContainer aboutCommandGrid">
        <div className="aboutCommandCopy commandReveal">
          ...
          <p><Link href="/product/hirescore-ai">HireScoreAI</Link> is built for teams that need a better way to source, evaluate and move technical talent from requirement to joining.</p>
          <p>Explore how the platform supports <Link href="/solutions/tech-hiring">technical hiring</Link>, <Link href="/solutions/bulk-resume-screening">high-volume candidate review</Link>, or different team environments through the <Link href="/solutions/hr-teams">HR team</Link>, <Link href="/solutions/startups">growing company</Link>, <Link href="/solutions/recruitment-agencies">recruitment agency</Link> and <Link href="/solutions/staffing-companies">staffing company</Link> solution pages.</p>
