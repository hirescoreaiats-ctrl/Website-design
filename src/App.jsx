...
const blogPosts = [
  {
    ...
    image: '/ai-action-agent-workflow.png',
...
function TechHiringHomePage() {
  return (
    <div className="commandHome techHome">
      ...
      <AboutCommandSection />
      <HomepageFaqSection />
      <section className="commandCta techFinalCta"><div className="commandContainer commandCtaBox"><span className="sectionKicker"><Sparkles size={13} /> Built for high-volume recruitment</span><h2>Turn Candidate Volume into <span>Qualified Shortlists</span></h2><p>Source candidates, screen resumes in bulk and manage IT and non-IT hiring pipelines in one HireScoreAI workspace.</p><div className="commandActions"><a className="commandButton commandButtonPrimary" href={PILOT_MAILTO}>Request Pilot Access <ArrowRight size={17} /></a><Link className="commandButton commandButtonGhost" href="/request-candidate-sourcing">Explore Candidate Sourcing</Link></div></div></section>
    </div>
  )
}
    ...
    <section className="techHero">
      <div className="commandContainer techHeroGrid">
        <div className="techHeroCopy">
          <span className="sectionKicker"><Sparkles size={13} /> AI Bulk Hiring &amp; Candidate Sourcing Platform</span>
          <h1>{HOME_H1}</h1>
          <p>HireScoreAI helps bulk hiring teams, staffing firms, recruitment agencies and HR teams source candidates, screen large resume volumes, rank talent by job fit and manage IT and non-IT hiring pipelines in one software product.</p>
          <div className="commandActions"><a className="commandButton commandButtonPrimary" href={PILOT_MAILTO}>Request Pilot Access <ArrowRight size={17} /></a><Link className="commandButton commandButtonGhost" href="/request-candidate-sourcing">Explore Candidate Sourcing</Link></div>
          <div className="techHeroNote"><ShieldCheck size={16} /> Product-powered hiring intelligence for IT roles, non-IT roles and every team in between.</div>
        </div>
        <div className="techCommandVisual heroProductShell" aria-label="Illustrative HireScoreAI hiring command center for an IT or non-IT role">
          ...
          <div className="heroProductMain">
            <header><div><small>Non-IT role · Illustrative demo</small><strong>Regional Sales Manager</strong></div><span><i /> Hiring</span></header>
...
function TechHiringProblem() {
  const pains = [[Files, 'High resume volumes slow down review'], [Target, 'Hard to identify actual role fit'], [CalendarCheck, 'Sourcing and hiring actions stay scattered']]
  return <section className="techStorySection techProblem"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker">The challenge</span><h2>Hiring gets complex across every industry.</h2><p>Whether the role is in technology, sales, finance, operations or another function, sourcing, resume review, interviews, follow-ups and decisions often happen across disconnected tools.</p></div><div className="techPainGrid">{pains.map(([Icon, text]) => <article key={text}><Icon size={20} /><span>{text}</span></article>)}</div><p className="techBridge"><Workflow size={18} /> HireScoreAI brings IT and non-IT recruitment into one connected hiring workflow.</p></div></section>
}

function HiringJourney() {
  const journey = [
    ['Requirement', 'Senior Software Engineer', 'Bengaluru · 5–8 years', BriefcaseBusiness], ['Sourcing', 'Candidate pipeline created', 'Candidates enter the role workspace', Network],
    ['Screening', 'Candidate intelligence applied', 'Role-fit evidence is ready to review', SearchCheck], ['Shortlisting', 'Strong candidates prioritized', 'The hiring team confirms progression', BadgeCheck],
    ['Interviews', 'Candidates progressing', 'Interview steps stay attached to each profile', CalendarCheck], ['Offer', 'Offers tracked', 'Outstanding decisions remain visible', MailCheck],
    ['Joining', 'Final candidate confirmed', 'The role journey closes with joining visibility', CheckCircle2],
  ]
  const [active, setActive] = useState(0)
  const ActiveIcon = journey[active][3]
  return <section className="techStorySection hiringJourney" id="hiring-journey"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker">Requirement-to-joining workflow</span><h2>From Requirement to Joining</h2><p>One connected hiring journey from the first requirement to the final joining.</p></div><div className="journeyShell"><div className="journeyStages" role="tablist" aria-label="Hiring journey stages">{journey.map(([label], index) => <button type="button" role="tab" aria-selected={active === index} className={active === index ? 'isActive' : ''} onClick={() => setActive(index)} key={label}><span>{String(index + 1).padStart(2, '0')}</span><strong>{label}</strong></button>)}</div><div className="journeyContext" role="tabpanel"><div className="journeyContextIcon"><ActiveIcon size={20}/></div><div><span>{journey[active][0]}</span><h3>{journey[active][1]}</h3><p>{journey[active][2]}</p></div><span className="journeyReady"><CheckCircle2 size={15}/> Active role workspace</span></div></div></div></section>
}

function TechCandidateIntelligence() {
  return <section className="techStorySection techIntelligence" id="candidate-intelligence"><div className="commandContainer techSplit"><div className="techSectionIntro"><span className="sectionKicker"><BrainCircuit size={13} /> Candidate Intelligence</span><h2>Know Who Deserves Attention—and Why</h2><p>HireScoreAI evaluates candidates against the role and turns resume data into explainable hiring intelligence.</p><div className="candidateRanking"><small>Candidate ranking</small>{[['1','Aditi Rao','91'],['2','Rahul Mehta','87'],['3','Neha Kapoor','84']].map(([rank,name,score])=><div key={name}><span>{rank}</span><strong>{name}</strong><b>{score}</b></div>)}</div></div><div className="intelligenceCard"><header><div><small>Role-fit analysis</small><strong>Senior Software Engineer</strong></div><span>Strong match</span></header><div className="intelligenceProfile"><span>AR</span><div><h3>Aditi Rao</h3><p>Platform engineering · 7 years</p></div><b>91<small>% role fit</small></b></div><div className="intelligenceEvidence"><div><small>Matched evidence</small><span>Distributed systems</span><span>API architecture</span><span>Technical ownership</span></div><div><small>Review context</small><p>Strong scalable backend and technical leadership experience. A preferred cloud certification is not listed.</p></div></div><div className="intelligenceReason"><BrainCircuit size={17}/><span><small>Why this candidate</small><strong>Evidence aligns with the role’s core architecture and ownership needs.</strong></span></div><footer><ArrowRight size={17}/><span><small>Recommended next step</small><strong>Advance to Technical Interview</strong></span></footer></div></div></section>
}

function TechActionAgent() {
  return <section className="techStorySection techActionSection" id="action-agent"><div className="commandContainer"><div className="techSectionIntro actionAgentIntro"><span className="sectionKicker"><Bot size={13} /> HireScoreAI Action Agent</span><h2>Move Hiring Forward Through Action</h2><p>Use natural language to interact with the HireScoreAI workspace, review candidates and prepare connected hiring actions.</p></div><div className="actionWorkspace"><div className="actionConversation"><header><span><Bot size={18} /></span><div><strong>HireScoreAI Action Agent</strong><small><i /> Senior Software Engineer workspace</small></div><em>Actions remain reviewable</em></header><div className="actionChatScroll"><div className="actionBubble isUser"><small>Hiring team</small><p>Create a Senior Software Engineer role using this JD.</p></div><div className="actionBubble isAgent"><small>Action Agent</small><p>Senior Software Engineer created. Public application page ready.</p><div className="agentInlineButtons"><button type="button">View Job</button><button type="button">Review Details</button></div></div><div className="actionBubble isUser"><small>Hiring team</small><p>Review the current applicants and show me the strongest candidates.</p></div><div className="actionBubble isAgent"><small>Action Agent</small><p>48 candidates analyzed. 12 strong matches identified.</p><div className="agentTopMatches">{[['Aditi Rao','91'],['Rahul Mehta','87'],['Neha Kapoor','84']].map(([name,score])=><span key={name}><strong>{name}</strong><b>{score}%</b></span>)}</div></div><div className="actionBubble isUser"><small>Hiring team</small><p>Move Aditi and Rahul to shortlist and prepare the next interview steps.</p></div><div className="actionBubble isAgent"><small>Action Agent</small><p>2 candidates prepared for shortlist. Interview communication is ready for review.</p><button className="agentConfirmButton" type="button"><ShieldCheck size={14}/> Review &amp; Confirm</button></div></div><footer><MessageSquareText size={15}/> Ask HireScoreAI… <ArrowRight size={14}/></footer></div><aside className="actionPipelineUpdate"><small>Workspace updates</small><div><BadgeCheck size={17}/><span><strong>Shortlisted</strong><small>14 → 16 candidates</small></span></div><div><CalendarCheck size={17}/><span><strong>Next actions</strong><small>2 interviews ready to confirm</small></span></div><div><GitBranch size={17}/><span><strong>Pipeline</strong><small>Awaiting team confirmation</small></span></div><Link href="/resources/blogs/hire-through-conversation-action-ai-agent">Explore the Action Agent <ArrowRight size={14}/></Link></aside></div></div></section>
}
...
function HiringCommandCenter() {
  ...
  return <section className="techStorySection commandCenterSection" id="hiring-pipeline"><div className="commandContainer pipelineSectionLayout"><div className="techSectionIntro"><span className="sectionKicker"><GitBranch size={13} /> Hiring pipeline</span><h2>Keep Every Candidate Moving</h2><p>See every candidate, hiring stage and outstanding action in one connected workspace—whatever the role or industry.</p><Link className="techTextLink" href="/product/hiring-pipeline">Explore the hiring pipeline <ArrowRight size={14}/></Link></div><div className="kanbanWorkspace"><header><div><small>Non-IT role pipeline · Illustrative sample</small><strong>Finance Analyst</strong></div><span><SearchCheck size={14}/> Search candidates</span><button type="button">Filter</button></header><div className="kanbanContent"><div className="kanbanBoard">{columns.map(([title,cards])=><div className="kanbanColumn" key={title}><header><strong>{title}</strong><span>{cards.length}</span></header>{cards.map(([name,score],index)=><article key={`${title}-${name}`}><div><span>{name.split(' ').map(part=>part[0]).join('')}</span><strong>{name}</strong></div><small>Finance Analyst</small><footer><b>{score}% match</b><em>{index===0?'Needs action':'In progress'}</em></footer></article>)}</div>)}</div><aside className="kanbanAttention"><small>Needs Attention</small><ul><li><span>Strong matches waiting for review</span><b>8</b></li><li><span>Interviews awaiting confirmation</span><b>3</b></li><li><span>Candidate follow-ups</span><b>2</b></li><li><span>Pending offer</span><b>1</b></li></ul></aside></div></div></div></section>
}

function HiringAnalytics() {
  return <section className="techStorySection hiringAnalyticsSection" id="hiring-analytics"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker"><BarChart3 size={13}/> Hiring Analytics</span><h2>Understand What Is Working—and Where Hiring Slows Down</h2><p>Compare candidate movement, stage performance, source quality and bottlenecks across IT and non-IT roles in one view.</p></div><div className="analyticsWorkspace"><header><div><small>Hiring analytics</small><strong>Performance overview</strong></div><nav><button className="isActive" type="button">Overview</button><button type="button">Roles</button><button type="button">Sources</button></nav><span>Illustrative workspace data</span></header><div className="analyticsWorkspaceGrid"><section className="analyticsFunnel"><small>Stage conversion</small><div>{[['Screened → Shortlisted','19%'],['Shortlisted → Interview','38%'],['Interview → Offer','33%'],['Offer → Joining','50%']].map(([label,value],index)=><span key={label} style={{'--funnel-size':`${96-index*9}%`}}><b>{value}</b><em>{label}</em></span>)}</div></section><section className="analyticsRoles"><small>Role performance</small>{[['Senior Software Engineer','16 shortlisted · 6 interviewing','88'],['Regional Sales Manager','9 shortlisted · 4 interviewing','68'],['Finance Analyst','7 shortlisted · 2 interviewing','52']].map(([role,detail,width])=><div key={role}><span><strong>{role}</strong><small>{detail}</small></span><i><b style={{width:`${width}%`}}/></i></div>)}</section><section className="analyticsHealth"><small>Pipeline health</small><ul><li><span>Waiting for review</span><b>8</b></li><li><span>Interviews pending</span><b>3</b></li><li><span>Offers pending</span><b>1</b></li><li><span>Inactive too long</span><b>4</b></li></ul></section><section className="analyticsTime"><small>Average time in stage</small><div>{[['Screening','1.4d'],['Shortlisting','2.1d'],['Interview','4.6d'],['Offer','3.2d']].map(([stage,time])=><span key={stage}><strong>{time}</strong><em>{stage}</em></span>)}</div></section><section className="analyticsSources"><small>Candidate intake sources</small><div>{[['Career Page','36%'],['Job Portals','30%'],['Referral','19%'],['Recruiter Upload','15%']].map(([source,value])=><span key={source}><strong>{source}</strong><i><b style={{width:value}}/></i><em>{value}</em></span>)}</div></section></div></div></div></section>
}

function JourneyCapabilities() {
  const groups = [['Set up', [[UsersRound, 'Candidate Sourcing', '/request-candidate-sourcing'],[BriefcaseBusiness, 'Job Creation', '/product/create-job'],[Globe2, 'Public Apply Pages', '/product/public-apply-page'],[Files, 'Candidate Intake', '/product/resume-upload']]],['Evaluate', [[SearchCheck, 'AI Resume Screening', '/product/ai-resume-parsing'],[Target, 'JD-Based Scoring', '/product/ai-candidate-scoring'],[Trophy, 'Candidate Ranking', '/product/ai-candidate-ranking'],[ClipboardCheck, 'AI Screening Tests', '/product/ai-screening-test']]],['Move', [[BadgeCheck, 'Shortlisting', '/product/ai-shortlisting'],[MessageSquareText, 'Candidate Communication', '/product/candidate-communication'],[CalendarCheck, 'Interview Scheduling', '/product/interview-scheduling']]],['Manage', [[GitBranch, 'Hiring Pipeline', '/product/hiring-pipeline'],[BarChart3, 'Hiring Analytics', '/product/hiring-pipeline'],[Bot, 'Action Agent', '/resources/blogs/hire-through-conversation-action-ai-agent']]]]
  return <section className="techStorySection techCapabilities"><div className="commandContainer"><div className="techSectionIntro"><span className="sectionKicker">Supporting product capabilities</span><h2>Everything needed to set up, evaluate, move and manage hiring.</h2><p>Explore the capabilities that support each part of the requirement-to-joining journey.</p></div><div className="capabilityGroups">{groups.map(([group,items])=><section key={group}><small>{group}</small><div>{items.map(([Icon,title,href])=><Link href={href} key={title}><Icon size={18}/><span>{title}</span><ArrowRight size={13}/></Link>)}</div></section>)}</div></div></section>
}

function HiringSupport() {
  return <section className="techStorySection hiringSupport"><div className="commandContainer hiringSupportBox"><div><span className="sectionKicker"><UsersRound size={13} /> Candidate sourcing + bulk screening</span><h2>Need More Candidates—or Faster Review at Scale?</h2><p>Share an active IT or non-IT requirement for candidate sourcing support, or bring your existing resume volume into HireScoreAI for AI screening, JD-based ranking and a structured shortlist. Every candidate stays connected to the same role pipeline.</p></div><Link className="commandButton commandButtonGhost" href="/request-candidate-sourcing">Explore Candidate Sourcing <ArrowRight size={16} /></Link></div></section>
}
    ...
    <section className="commandSection homeFaqSection" id="home-faq" aria-labelledby="home-faq-title">
      <div className="commandContainer">
        <div className="commandHeading commandReveal">
          <h2 id="home-faq-title">HireScoreAI recruitment platform FAQs</h2>
          <p>Clear answers about IT and non-IT hiring, sourcing, AI candidate intelligence, the Action Agent and connected workflows. Explore <Link href="/resources">recruitment resources</Link> and <Link href="/resources/blogs">hiring insights</Link>, review <Link href="/pricing">HireScoreAI pricing</Link>, or <Link href="/contact">contact our team</Link>.</p>
        </div>
    ...
    <section className="commandSection aboutCommandSection" id="about">
      <div className="commandContainer aboutCommandGrid">
        <div className="aboutCommandCopy commandReveal">
          ...
          <h2>Built for a faster, clearer and more connected <span>recruitment journey.</span></h2>
          <p><Link href="/product/hirescore-ai">HireScoreAI</Link> is built for teams hiring across IT and non-IT functions—from software and data to sales, finance, operations, healthcare and manufacturing.</p>
          <p>The platform connects candidate sourcing, role-specific evaluation, hiring actions, pipeline management and analytics from requirement to joining—including dedicated support for <Link href="/solutions/tech-hiring">IT and tech hiring</Link>—while every hiring decision remains with your team.</p>
          <div className="aboutFounderIdentity" id="sachin-yadav">
            <span><UsersRound size={18} /></span>
            <div><small>Founder, HireScoreAI</small><strong>Sachin Yadav</strong><p>Building HireScoreAI around one idea: reduce repetitive hiring work, make candidate decisions clearer and keep the hiring journey connected from requirement to joining.</p></div>
          </div>
        </div>
        ...
        <header className="actionArticleHero">
          <div className="actionArticleShell actionArticleHeroGrid">
            ...
            <div className="actionHeroVisual">
              <img
                src={post.image}
                alt="HireScoreAI Action AI Agent workflow showing Ask, Review, Approve and Results steps"
                width="1402"
                height="1122"
                fetchpriority="high"
              />
    ...
    <footer className="commandFooter professionalFooter">
      <div className="commandContainer professionalFooterTop">
        <div className="footerBrandBlock">
          ...
          <p>Source, screen and hire IT and non-IT talent through one connected HireScoreAI workflow.</p>
